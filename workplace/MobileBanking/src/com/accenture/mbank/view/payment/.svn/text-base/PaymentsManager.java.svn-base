...
import com.accenture.mbank.logic.CheckBillPaymentJson;
...
import com.accenture.mbank.logic.InsertBillPaymentJson;
...
import com.accenture.mbank.view.adapter.CoverFlowViewAdapter;
import com.accenture.mbank.view.payment.ResizeLayout.OnResizeListener;
 ...
 * @author junxu.wang
 */
public class PaymentsManager implements OnClickListener, OnItemSelectedListener,
        OnItemClickListener, OnCoverFlowItemSelectedListener {
     ...
     */
    public static final int BANK_TRANSFER = 1, TRANSFER_ENTRY = 2, CARD_TOP_UP = 3,
            PHONE_TOP_UP = 4,PRECOMPILED_BILL=5,BLANK_BILL=6,MAV_RAV=7, PTYPE_NULL = 0;

    private static final int STEP_1 = R.id.new_payments_step1_rb,
            STEP_2 = R.id.new_payments_step2_rb, STEP_3 = R.id.new_payments_step3_rb;

    public static final int FORM_DEFAULT = 0, FORM_RECENT_PAYMENT = 1, FORM_ACCOUNT = 2;
    ...
    private CustomEditText amount_et;
    ...
    private int curStep, curPaymentType, curPayerAccoutSeltectId, curPayeeSelectId;
    ...
    public static final int VERIFYCARD_INITAL = 0, VERIFYCARD_FAILED = 1, VERIFYCARD_SUCCESS = 2;
    ...
    private TextView payer_detail_title_tv, payer_detail_context_tv, payee_detail_title_tv,
            payee_detail_context_tv;
    ...
    /**
     * @param showForm {@link #FORM_DEFAULT} {@link #FORM_RECENT_PAYMENT}
     *            {@link #FORM_ACCOUNT}
     * @param account the payer
     * @param destAccount the payee
     * @param time execution date
     * @param amount amount for payment
     * @param paymentType {@link #BANK_TRANSFER} {@link #TRANSFER_ENTRY}
     *            {@link #CARD_TOP_UP} {@link #PHONE_TOP_UP}
     */
    public void setRecover(int showForm, AccountsModel account, DestaccountModel destAccount,
            long time, double amount, int paymentType) {
    ...
    public void setRecentRecover(int showForm, RecentTransferModel recentTransferModel,
        ...
        if (Contants.SIM_TOP_UP.equals(recentTransferModel.getType())) {
            ...
            curPayee = pr;
        } else if (Contants.TRANSFER_ENTRY.equals(recentTransferModel.getType())) {
            ...
            curPayee = dm;
        } else if (Contants.BANK_TRANSFER.equals(recentTransferModel.getType())) {
            ...
            // curPayee = br;
        } else if (Contants.PREPAID_CARD_RELOAD.equals(recentTransferModel.getType())) {
    ...
    public ViewGroup init() {
        layout = (ViewGroup)activity.findViewById(R.id.new_payments);
        ...
        new_payments1_clu = (LinearLayout)layout.findViewById(R.id.new_payments1_clu);

        new_payments_step_rg = (RadioGroup)layout.findViewById(R.id.new_payments_step_rg);
        ...
        type_payment_ad = new ArrayAdapter<String>(activity, android.R.layout.simple_spinner_item,
                activity.getResources().getStringArray(R.array.new_payments_types));
        ...
        account_folders_cf = (CoverFlow)activity.findViewById(R.id.account_folders_cf);
        account_folders_cf.setAdapter(account_folders_cf_ad);
        account_folders_cf.setAnimationDuration(500);
        account_folders_cf.setOnItemClickListener(this);
        account_folders_cf.setOnCoverFlowItemSelectedListener(this);

        phone_payee_folders_cf = (CoverFlow)activity.findViewById(R.id.phone_payee_folders_cf);
        phone_payee_folders_cf.setAdapter(phone_payee_cf_ad);
        phone_payee_folders_cf.setAnimationDuration(500);
        phone_payee_folders_cf.setOnItemClickListener(this);
        phone_payee_folders_cf.setOnCoverFlowItemSelectedListener(this);

        payee_folders_cf = (CoverFlow)activity.findViewById(R.id.payee_folders_cf);
        // payee_folders_cf.setAdapter(card_folders_cf_ad);
        payee_folders_cf.setAnimationDuration(500);
        payee_folders_cf.setOnItemClickListener(this);
        payee_folders_cf.setOnCoverFlowItemSelectedListener(this);

        amountbar_lin = (LinearLayout)layout.findViewById(R.id.amountbar_lin);
        amountbar_lin.setOnClickListener(this);
        amount_et = (CustomEditText)layout.findViewById(R.id.amount_et);
        ...
        amount_et.setFilters(new InputFilter[] {
        ...
        top_up_amount_gv = (RadioGroup)layout.findViewById(R.id.top_up_amount_gv);

        execution_date_lin = (LinearLayout)layout.findViewById(R.id.execution_date_lin);
        execution_date_et = (EditText)layout.findViewById(R.id.execution_date_et);
        ...
        payer_detail_title_tv = (TextView)layout.findViewById(R.id.payer_detail_title_tv);
        payer_detail_context_tv = (TextView)layout.findViewById(R.id.payer_detail_context_tv);
        payee_detail_title_tv = (TextView)layout.findViewById(R.id.payee_detail_title_tv);
        payee_detail_context_tv = (TextView)layout.findViewById(R.id.payee_detail_context_tv);
        ...
        mPaymentsManagerBill=new PaymentsManagerBill(activity,new_payment_bill);
        
        new_payment_blank_bill=(ViewGroup) layout.findViewById(R.id.payment_blank_bill);
        mPaymentsManagerBlankBill=new PaymentsManagerBlankBill(activity,new_payment_blank_bill);
        
        new_payment_mav_rav_bill=(ViewGroup) layout.findViewById(R.id.payment_mav_rav);
        mPaymentsManagerMAVRAV=new PaymentsManagerMAVRAV(activity,new_payment_mav_rav_bill);
        return layout;
    }
    ...
    private void reset() {
        ...
        if (showForm == PaymentsManager.FORM_RECENT_PAYMENT) {
            ...
            if (getPaymentType() != CARD_TOP_UP && getPaymentType() != PHONE_TOP_UP) {
            ...
            AccountsModel am = generateAccountModel();
...
//                payment_account_info_tv.setText(am.getCardName() + ", " + am.getIbanCode() + ", "
//                        + am.getAccountId());

                TextView payment_amount_tv = (TextView)layout.findViewById(R.id.payment_amount_tv);
                payment_amount_tv.setText(Utils.generateFormatMoney("$", generateAmount()));
                ...
                switch (getPaymentType()) {
                    case BANK_TRANSFER:
                        ...
                        DestaccountModel br = generateDestAccountModel();
                        if(br==null){
                        	Toast.makeText(activity, "select correct payee!", Toast.LENGTH_SHORT).show();
                        	return ;
                        }
                    ...
                    case PRECOMPILED_BILL:
                    	payee_account_type_tv = (TextView)layout
                        .findViewById(R.id.payee_account_type_tv);
                    	payee_account_type_tv.setText(mPaymentsManagerBill.getPayableTo());
                    	payee_account_info_tv = (TextView)layout
                    	.findViewById(R.id.payee_account_info_tv);
                    	payee_account_info_tv.setText(mPaymentsManagerBill.getBillNumber());
                    	break;
                    case BLANK_BILL:
                    case MAV_RAV:
                    	payee_account_type_tv = (TextView)layout
            ...
            case STEP_3:
                ...
                if (confirmResult) {
                    ...
                    Drawable icon = activity.getResources().getDrawable(R.drawable.ok);
                    ...
                    Drawable icon = activity.getResources().getDrawable(R.drawable.error);
                    icon.setBounds(0, 0, icon.getMinimumWidth(), icon.getMinimumHeight());
                    result_tv.setCompoundDrawables(icon, null, null, null);
                }
                break;
    ...
    private String getTransferDetails() {
        ...
        String amount = Utils.generateFormatMoney("$", generateAmount());
        String fees = Utils.generateFormatMoney("$", this.fee);
        switch (getPaymentType()) {
            case BANK_TRANSFER:
                DestaccountModel dm = (DestaccountModel)tPayee;
                return String.format(detailsFormateBankTransfer,
                        res.getString(R.string.type_of_payment1),
                        paymentType,
                        res.getString(R.string.account1),
                        account, 
                        res.getString(R.string.date1), 
                        TimeUtil.getDateString(generateDate(), TimeUtil.dateFormat1),
                        res.getString(R.string.amount1),
                        amount,
                        res.getString(R.string.payee1),
                        dm.getTitle(), 
                        res.getString(R.string.iban), 
                        dm.getIban(),
                        res.getString(R.string.reason),
                        generateDescription(),
                        res.getString(R.string.fees),
                        fees
                        );
            case TRANSFER_ENTRY:
                dm = (DestaccountModel)tPayee;
                return String.format(detailsFormateTransferEntry,
                        res.getString(R.string.type_of_payment1),
                        paymentType,
                        res.getString(R.string.account1),
                        account,
            ...
            case CARD_TOP_UP:
                ...
                return String.format(detailsFormateCardTopUp,
                        ...
                        res.getString(R.string.account1),
                        account, 
//                        res.getString(R.string.date1), TimeUtil.getDateString(generateDate(), TimeUtil.dateFormat1),
                        res.getString(R.string.amount1),
                        amount, 
                        res.getString(R.string.payee1),
                        cr.getName(), 
            ...
            case PRECOMPILED_BILL:
            return String.format(detailsFormatePrecompiledBill,
                    res.getString(R.string.type_of_payment1),
                    paymentType,
                    res.getString(R.string.account1),
                    account, 
                    res.getString(R.string.bill_holder1),
                    mPaymentsManagerBill.getBillHolderName(),
                    res.getString(R.string.account_number1),
                    mPaymentsManagerBill.getAccountNumber(),
                    res.getString(R.string.amount1),
                    amount,
                    res.getString(R.string.bank_free1),
                    Utils.generateFormatMoney("$", this.mCheckBillPaymentResponseModel.getCheckBillPaymentValueModel().getCharges()),
                    res.getString(R.string.postal_free1),
                    Utils.generateFormatMoney("$", this.mCheckBillPaymentResponseModel.getCheckBillPaymentValueModel().getPostalCharges())
                    );
            case BLANK_BILL:
            	 return String.format(detailsFormatePrecompiledBill,
                         res.getString(R.string.type_of_payment1),
...
//        }
    ...
    private void inviliAccountView(int paymentsType) {
        ...
        invilidataPayees();
        if (showForm == FORM_ACCOUNT&&Contants.transferEntryAccounts!=null) {
            List<AccountsModel> ens=Contants.transferEntryAccounts.get(0).getAccounts();
            int count=-1;
            String src,des=null;
            if(ens!=null){
                count=ens.size();
            }
            if(generateAccountModel()!=null){
                des=generateAccountModel().getAccountCode();
            }
            ...
            switch (paymentsType) {
                default:
                    ...
                    if (allAccounts != null&&allAccounts.size()>0) {
                        account_folders_cf_ad.setDatas(allAccounts.get(0).getAccounts());
...
//                        loadAccountData(GET_ACCOUNT);
                ...
                case BANK_TRANSFER:
                    if (Contants.bankTransferAccounts != null) {
                        account_folders_cf_ad.setDatas(Contants.bankTransferAccounts.get(0).getAccounts());
                        account_folders_cf_ad.notifyDataSetChanged();
                    } else {
//                        loadAccountData(GET_BANK_TRANSFER);
                    ...
                    if (Contants.getRecipientListModel != null) {
                        bank_payee_cf_ad.setDatas(Contants.getRecipientListModel.getBankRecipientList());
                    ...
                    break;
                case TRANSFER_ENTRY:
                    if (null != Contants.transferEntryAccounts) {
                        account_folders_cf_ad.setDatas(Contants.transferEntryAccounts.get(0).getAccounts());
                        account_folders_cf_ad.notifyDataSetChanged();

                        entry_payee_cf_ad.setDatas(Contants.transferEntryAccounts.get(0).getAccounts());
...
//                        loadAccountData(GET_TRANSFER_ENTRY);
                    ...
                    break;
                case CARD_TOP_UP:
                    if (null != Contants.chargeAccounts) {
                        account_folders_cf_ad.setDatas(Contants.chargeAccounts.get(0).getAccounts());
                        account_folders_cf_ad.notifyDataSetChanged();
                    } else {
//                        loadAccountData(GET_CARD_TOP_UP);
                    ...
                    if (Contants.getRecipientListModel != null) {
                        card_payee_cf_ad.setDatas(Contants.getRecipientListModel.getCardRecipientList());
                    ...
                    break;
                case PHONE_TOP_UP:
                    if (null != Contants.simTopUpAccounts) {
                        account_folders_cf_ad.setDatas(Contants.simTopUpAccounts.get(0).getAccounts());
                        account_folders_cf_ad.notifyDataSetChanged();
                    } else {
//                        loadAccountData(GET_PHONE_TOP_UP);
                    ...
                    if (Contants.getRecipientListModel != null) {
                        phone_payee_cf_ad.setDatas(Contants.getRecipientListModel.getPhoneRecipientList());
                        phone_payee_folders_cf.setAdapter(phone_payee_cf_ad);
                    }
            ...
            switch (msg.what) {
                ...
                case GET_ACCOUNT:
                    ...
                    if (allAccounts != null && allAccounts.get(0) != null) {
                        List<AccountsModel> accounts = allAccounts.get(0).getAccounts();
                        int size = accounts.size();
                        List<AccountsModel> destLs = null;

                        if (Contants.transferEntryAccounts == null) {
                            Contants.transferEntryAccounts = new ArrayList<AccountsForServiceModel>();
                            ...
                            Contants.transferEntryAccounts.add(mAccountsForServiceModel);
                        } else if (Contants.transferEntryAccounts.get(0) == null) {
                            AccountsForServiceModel mAccountsForServiceModel = new AccountsForServiceModel();
                            destLs = new ArrayList<AccountsModel>();
                            mAccountsForServiceModel.setAccounts(destLs);
                            Contants.transferEntryAccounts.add(mAccountsForServiceModel);
                        } else if (destLs == null) {
                            destLs = Contants.transferEntryAccounts.get(0).getAccounts();
                            if (destLs == null) {
                                destLs = new ArrayList<AccountsModel>();
                                Contants.transferEntryAccounts.get(0).setAccounts(destLs);
                            } else {
                                destLs.clear();
                            }
                        }
...
//	                    }
                    Contants.bankTransferAccounts = allAccounts;

                    Contants.chargeAccounts = allAccounts;

                    Contants.simTopUpAccounts = allAccounts;

                    if(showForm==FORM_DEFAULT&&(Contants.bankTransferAccounts==null||Contants.bankTransferAccounts.size()<=0||Contants.bankTransferAccounts.get(0).getAccounts().size()==0)){
                    	payment_type_sp.setEnabled(false);
                    	Toast.makeText(activity, "none payer account!", Toast.LENGTH_LONG).show();
                    }else
                    if(Contants.bankTransferAccounts!=null&&Contants.bankTransferAccounts.size()>0&&Contants.bankTransferAccounts.get(0).getAccounts().size()<=1){
                    		//If the customer has only one account, the "transfer entry" option must be not visible: the customer hasn't to select the payment type "transfer entry". 
                    		type_payment_ad = new ArrayAdapter<String>(activity, android.R.layout.simple_spinner_item,
                                    activity.getResources().getStringArray(R.array.new_payments_types1));
                            type_payment_ad.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item);
                            payment_type_sp.setAdapter(type_payment_ad);
                    	}else{
                        		type_payment_ad = new ArrayAdapter<String>(activity, android.R.layout.simple_spinner_item,
                ...
                case GET_BANK_TRANSFER:
                    Contants.bankTransferAccounts = (List<AccountsForServiceModel>)msg.obj;
                    inviliAccountView(BANK_TRANSFER);

                    downloading_pd.dismiss();
                    break;
                case GET_TRANSFER_ENTRY:
                    Contants.transferEntryAccounts = (List<AccountsForServiceModel>)msg.obj;
                ...
                case GET_CARD_TOP_UP:
                    Contants.chargeAccounts = (List<AccountsForServiceModel>)msg.obj;
                    inviliAccountView(CARD_TOP_UP);

                    downloading_pd.dismiss();
                    break;
                case GET_PHONE_TOP_UP:
                    Contants.simTopUpAccounts = (List<AccountsForServiceModel>)msg.obj;
                    inviliAccountView(PHONE_TOP_UP);

                    downloading_pd.dismiss();
                    break;
                case GET_RECIPIENTLISTMODEL:
                    Contants.getRecipientListModel = (GetRecipientListModel)msg.obj;
                    invilidataPayees();
                    downloading_pd.dismiss();
                    break;
                case GET_ASKPIN:
                    ...
                    if (generateOtp.responsePublicModel.isSuccess()) {
                        if(MainActivity.setting.getChannelToRecelvePin()==SettingModel.EMAIL){
                            Toast.makeText(activity, "ask pin success, please get pin by email",
                                    Toast.LENGTH_LONG).show();
                        }else if(MainActivity.setting.getChannelToRecelvePin()==SettingModel.SMS){
                            Toast.makeText(activity, "ask pin success, please get pin by sms",
                                    Toast.LENGTH_LONG).show();
                        }
...
//                            fee = mCheckBillPaymentResponseModel.getCharges();
                            if (mCheckBillPaymentResponseModel.responsePublicModel.isSuccess()) {
                            	if ("91082"
                ...
                case CONFIRM:
                    ...
                    if(confirmResult){
                        new PostThread(activity, null, allAccounts, mHandler, PaymentsManager.TYPE_UPDATE_BALANCE,
                    ...
                    if (companyAmount.responsePublicModel.isSuccess()) {
                        ...
                        if (mAmountAvailableLs == null || mAmountAvailableLs.size() <= 0) {
                            DialogManager.createMessageDialog(
                                    companyAmount.responsePublicModel.getResultDescription(),
                                    activity).show();
                        } else {
                ...
                case VERTIFY_CARD:
                    ...
                    if (list != null && list.size() > 0) {
                        ...
                        activity.displayErrorMessage(activity.getString(R.string.no_card_found));
                    }
                    break;
    ...
    public void initData() {
        // bankTransferAccounts=Contants.bankTransferAccounts;
        // transferEntryAccounts=Contants.transferEntryAccounts;
        // chargeAccounts=Contants.simTopUpAccounts;
        // simTopUpAccounts=Contants.chargeAccounts;
        // getRecipientListModel=Contants.getRecipientListModel;

        // new PostThread(activity, null, null, mHandler,
        // PaymentsManager.GET_RECIPIENTLISTMODEL,
        // PostThread.TYPE_RECIPIENTLISTMODEL).start();
    	allAccounts=Contants.bankTransferAccounts;
       if(showForm==FORM_RECENT_PAYMENT){
            if(getPaymentType()==PHONE_TOP_UP){
                loadAmountAvilable();
            }
        }else{
        	if (Contants.bankTransferAccounts == null || Contants.bankTransferAccounts.size() <= 0||Contants.bankTransferAccounts.get(0).getAccounts()==null||Contants.bankTransferAccounts.get(0).getAccounts().size()<=0){
            		loadAccountData(GET_ACCOUNT);
            }else{
            	//If the customer has only one account, the "transfer entry" option must be not visible: the customer hasn't to select the payment type "transfer entry". 
            	if(Contants.bankTransferAccounts!=null&&Contants.bankTransferAccounts.size()>0&&Contants.bankTransferAccounts.get(0).getAccounts().size()<=1){
    ...
    protected void displayPaymentRecap() {
        ...
        builder.setPositiveButton(R.string.ask_pin, new DialogInterface.OnClickListener() {

            @Override
            public void onClick(DialogInterface dialog, int which) {
                downloading_pd.show();
                new PostThread(activity, null, null, mHandler, PaymentsManager.GET_ASKPIN,
...
//                displayPaymentRecap();
    ...
    private void showPaymentType(int visibility, int selectIndex) {
        ...
        if(Contants.bankTransferAccounts!=null&&Contants.bankTransferAccounts.size()<=1&&selectIndex==TRANSFER_ENTRY){
        	--selectIndex;
        }
    ...
    private View showPayer(int visibility) {
        View view=null;
        if (showForm == FORM_RECENT_PAYMENT || showForm == FORM_ACCOUNT) {
    ...
    private void showBillLayout(int visibility){
    	...
    	mPaymentsManagerBill.setBillHolderName(Contants.getUserInfo.getCustomerName()+" "+ Contants.getUserInfo.getCustomerSurname());
    }
    
    private void showBlackBillLayout(int visibility){
    	mPaymentsManagerBlankBill.setVisibility(visibility);
    	mPaymentsManagerBlankBill.setBillHolderName(Contants.getUserInfo.getCustomerName()+" "+ Contants.getUserInfo.getCustomerSurname());
    }
    
    private void showMavRavLayout(int visibility){
    	mPaymentsManagerMAVRAV.setVisibility(visibility);
    	mPaymentsManagerMAVRAV.setBillHolderName(Contants.getUserInfo.getCustomerName()+" "+ Contants.getUserInfo.getCustomerSurname());
    }
    ...
    private View showDate(int visibility, long timeInMillis) {
        if (timeInMillis == 0) {
        	execution_date_et.setText(TimeUtil.getDateString(System.currentTimeMillis(), TimeUtil.dateFormat8));
            setDate(System.currentTimeMillis());
        } else {
            execution_date_et.setText(TimeUtil.getDateString(mTimeInMillis, TimeUtil.dateFormat8));
        }
    ...
    private View showAmount(int visibility, double amount) {
        ...
        if (getPaymentType() == PHONE_TOP_UP) {
            if (amount > 0 && mAmountAvailableLs != null && mAmountAvailableLs.size() > 0) {
            ...
            if (amount == 0) {
                ...
                amount_et.setText(formateAmount(amount));
            }
        }
        return v;
    }
    ...
    private double generateAmount() {
        ...
        switch (getPaymentType()) {
            ...
            case CARD_TOP_UP:
                try {
                    String amountStr = amount_et.getText().toString().trim();
    ...
    /**
     * @param stepId R.id.new_payments_step1_rb,R.id.new_payments_step2_rb or
    ...
    private String generatePaymentTypeName() {
        if(showForm==FORM_RECENT_PAYMENT){
            return activity.getResources().getStringArray(R.array.new_payments_types)[getPaymentType()];
        }
    ...
    private int getPaymentType() {
        ...
        if (showForm != PaymentsManager.FORM_RECENT_PAYMENT) {
        	...
        	//If the customer has only one account, the "transfer entry" option must be not visible: the customer hasn't to select the payment type "transfer entry". 
        	if(Contants.bankTransferAccounts!=null&&Contants.bankTransferAccounts.size()<=1&&paymentType>=2){
        		++paymentType;
        	}
        }
        return paymentType;
    ...
    public class PostThread extends Thread {
        public static final int TYPE_DEF = 0, TYPE_BANK_TRANSFER = 1, TYPE_TRANSFER_ENTRY = 2,
                TYPE_CARD_TOP_UP = 3, TYPE_PHONE_TOP_UP = 4, TYPE_RECIPIENTLISTMODEL = 5,
                TYPE_ASKPIN = 6, TYPE_VALIDATE = 7, TYPE_CONFIRM = 8,
                TYPE_GET_AMOUNT_AVAILABLE = 9, TYPE_VERTIFY_CARD = 10, TYPE_INSERTRECIPIENT = 11,
                TYPE_GET_INIT_DATA = 12, TYPE_GET_ACCOUNT = 13,TYPE_UPDATE_BALANCE=14;
        ...
        public void run() {
            ...
            switch (type) {
                ...
                case TYPE_GET_ACCOUNT:
                    resultData = NewPaymentDataUtils.geBankTransferAccountst(activity);//getAccountsByService(activity,ServiceCode.SERVER_CODES);
                    Contants.getRecipientListModel = NewPaymentDataUtils.getRecipientList(activity);
                    break;
                case TYPE_UPDATE_BALANCE:
                	NewPaymentDataUtils.getBalance(activity,((List<AccountsForServiceModel>)postData).get(0).getAccounts(),
                            null);
               	break;
                case TYPE_BANK_TRANSFER:
                	resultData = NewPaymentDataUtils.geBankTransferAccountst(activity);
                    break;
                case TYPE_TRANSFER_ENTRY:
                    resultData =NewPaymentDataUtils.getTransferEntryAccounts(activity);
                    break;
                ...
                case TYPE_VERTIFY_CARD:
                    resultData = vertifyCard(mContext, generateAccountModel().getAccountCode(),
                            (CardRecipient)getPayee());
                    break;
    ...
    public boolean onDestroyed(){
        ...
        if(Contants.bankTransferAccounts!=null){
            Contants.bankTransferAccounts.clear();
            Contants.bankTransferAccounts=null;
        }
        if(Contants.transferEntryAccounts!=null){
            Contants.transferEntryAccounts.clear();
            Contants.transferEntryAccounts=null;
        }
        if(Contants.chargeAccounts!=null){
            Contants.chargeAccounts.clear();
            Contants.chargeAccounts=null;
        }
        if(Contants.simTopUpAccounts!=null){
            Contants.simTopUpAccounts.clear();
            Contants.simTopUpAccounts=null;
        }
        if(Contants.getRecipientListModel!=null){
            if(Contants.getRecipientListModel.getBankRecipientList()!=null){
                Contants.getRecipientListModel.getBankRecipientList().clear();
            }
            if(Contants.getRecipientListModel.getCardRecipientList()!=null){
                Contants.getRecipientListModel.getCardRecipientList().clear();
            }
            if(Contants.getRecipientListModel.getPhoneRecipientList()!=null){
                Contants.getRecipientListModel.getPhoneRecipientList().clear();
            }
            Contants.getRecipientListModel=null;
        }
        return true;
    }
...
//    private List<AccountsForServiceModel> bankTransferAccounts=Contants.bankTransferAccounts;

//    private List<AccountsForServiceModel> transferEntryAccounts=Contants.transferEntryAccounts;

//    private List<AccountsForServiceModel> simTopUpAccounts=Contants.simTopUpAccounts;

//    private List<AccountsForServiceModel> chargeAccounts=Contants.chargeAccounts;

//    private List<AccountsForServiceModel> transferEntryPayeeAccounts=Contants.transferEntryPayeeAccounts;

//    private GetRecipientListModel getRecipientListModel=Contants.getRecipientListModel;
    ...
    private CompanyAmountResponseModel getAmountAvailableLs() {
        ...
        String postData = CompanyAmountJson.CompanyAmountReportProtocal(
                accountsModel.getAccountCode(), pr.getProvider(), pr.getPhoneNumber(),
    ...
    private AccountsModel generateAccountModel() {
        if (showForm == PaymentsManager.FORM_RECENT_PAYMENT
    ...
    private Object getPayee() {
        ...
        if (showForm == PaymentsManager.FORM_RECENT_PAYMENT) {
            ...
            switch (getPaymentType()) {
                case BANK_TRANSFER:
                    BankRecipient br;
                    if (payee_folders_cf.getCount() - payee_folders_cf.getSelectedItemPosition()== 1)
                    	br = (BankRecipient)tmp_payee_phone;
                    else if (payee_folders_cf.getCount() - payee_folders_cf.getSelectedItemPosition()== 2)
                    	br = (BankRecipient)tmp_payee;
                    else
                    	br = (BankRecipient)payee_folders_cf.getCoverFlowSelectedItem();
                    if(br!=null){
                    ...
                    break;
                case TRANSFER_ENTRY:
                    AccountsModel am = (AccountsModel)payee_folders_cf.getCoverFlowSelectedItem();
                	DestaccountModel destAccount = new DestaccountModel();
                ...
                case CARD_TOP_UP:
                    CardRecipient cr;
                    if (payee_folders_cf.getCount() - payee_folders_cf.getSelectedItemPosition() > 1)
                        cr = (CardRecipient)payee_folders_cf.getCoverFlowSelectedItem();
                    else
                ...
                case PHONE_TOP_UP:
                    PhoneRecipient pr;
                    if (phone_payee_folders_cf.getCount() - curPayeeSelectId > 1){
                        CoverFlowViewAdapter coverFlowViewAdapter = (CoverFlowViewAdapter)phone_payee_folders_cf.getAdapter();
                        pr = (PhoneRecipient)coverFlowViewAdapter.getDatas().get(curPayeeSelectId);
                    }
                    else
    ...
    private DestaccountModel generateDestAccountModel() {
        ...
        if (tmpPayee instanceof CardRecipient) {
    ...
    private AmountAvailable generateAmountAvailable() {
        ...
        if (mAmountAvailableLs != null && index >= 0 && index < mAmountAvailableLs.size())
            return mAmountAvailableLs.get(index);
        return null;
    }

    private List<AmountAvailable> generateAmountAvailableList(AmountAvailable amountAvailable) {
    ...
    private CheckTransferResponseModel validateBankTransfer() {
        ...
        String postData = CheckTransferJson.CheckTransferReportProtocal(
                ...
                dateString, destAccount, Contants.publicModel, "1000");
    ...
    public static GetCardsResponseModel vertifyCard(Context activity, String accountCode,
            CardRecipient cr) {

        String postData = GetCardsJson.GetCardsReportProtocal(Contants.publicModel, cr.getName(),
                cr.getLast4Digits(), accountCode);
        HttpConnector httpConnector = new HttpConnector();
        String httpResult = httpConnector
                .requestByHttpPost(Contants.mobile_url, postData, activity);
    ...
    private CheckRechargeCardResponseModel validateCard() {
        ...
        String postData = CheckRechargeCardJson.CheckRechargeCardReportProtocal(
                ...
                tmp_InfoCardsModel.getTitle(), cr.getName(), "1000");
        HttpConnector httpConnector = new HttpConnector();
        String httpResult = httpConnector
                .requestByHttpPost(Contants.mobile_url, postData, activity);
    ...
    private CheckSimTopUpResponseModel validateSimTopUp() {
        ...
        String postData = CheckSimTopUpJson.CheckSimTopUpReportProtocal(Contants.publicModel,
                amountAvailableList, accountsModel.getAccountCode(), pr.getProvider(),
                pr.getPhoneNumber(), "1000");
        HttpConnector httpConnector = new HttpConnector();
        String httpResult = httpConnector
                .requestByHttpPost(Contants.mobile_url, postData, activity);
    ...
    private Object validate() {
        switch (getPaymentType()) {
            ...
            case PRECOMPILED_BILL:
            	...
            	checkBillPaymentValue.setAccountCode(accountsModel.getAccountCode());
            	checkBillPaymentValue.setPurposeDescription("N/A");
            	checkBillPaymentValue.setPostalAccount(mPaymentsManagerBill.getAccountNumber());
            	checkBillPaymentValue.setAmount(mPaymentsManagerBill.getAmount());
            	checkBillPaymentValue.setBillNumber(mPaymentsManagerBill.getBillNumber());
            	checkBillPaymentValue.setBillType(mPaymentsManagerBill.getType());
            	checkBillPaymentValue.setHolderName(mPaymentsManagerBill.getBillHolderName());
            	...
            	checkBillPaymentValue.setDueDate(TimeUtil.getDateString(System.currentTimeMillis(), TimeUtil.dateFormat2));
            	
            	String json=CheckBillPaymentJson.CheckBillPaymentReportProtocal(Contants.publicModel, checkBillPaymentValue);
            	json=post(json);
            	mCheckBillPaymentResponseModel =CheckBillPaymentJson.ParseCheckBillPaymentResponse(json);
            	return mCheckBillPaymentResponseModel;
            case BLANK_BILL:
                accountsModel = generateAccountModel();
            	checkBillPaymentValue=new CheckBillPaymentValueModel();
            	checkBillPaymentValue.setAccountCode(accountsModel.getAccountCode());
            	checkBillPaymentValue.setPostalAccount(mPaymentsManagerBlankBill.getAccountNumber());
            	checkBillPaymentValue.setAmount(mPaymentsManagerBlankBill.getAmount());
            	checkBillPaymentValue.setBillType(mPaymentsManagerBlankBill.getType());
            	checkBillPaymentValue.setHolderName(mPaymentsManagerBlankBill.getBillHolderName());
            	checkBillPaymentValue.setPurposeDescription(mPaymentsManagerBlankBill.getDescription());
            	checkBillPaymentValue.setCurrency("$");
            	...
            	checkBillPaymentValue.setDueDate(TimeUtil.getDateString(System.currentTimeMillis(), TimeUtil.dateFormat2));
            	json=CheckBillPaymentJson.CheckBillPaymentReportProtocal(Contants.publicModel, checkBillPaymentValue);
            	json=post(json);
            	mCheckBillPaymentResponseModel =CheckBillPaymentJson.ParseCheckBillPaymentResponse(json);
            	return mCheckBillPaymentResponseModel;
            case MAV_RAV:
            	...
            	checkBillPaymentValue=new CheckBillPaymentValueModel();
            	checkBillPaymentValue.setAccountCode(accountsModel.getAccountCode());
            	checkBillPaymentValue.setPostalAccount(mPaymentsManagerMAVRAV.getAccountNumber());
            	checkBillPaymentValue.setAmount(mPaymentsManagerMAVRAV.getAmount());
            	checkBillPaymentValue.setBillNumber(mPaymentsManagerMAVRAV.getBillNumber());
            	checkBillPaymentValue.setBillType(mPaymentsManagerMAVRAV.getType());
            	checkBillPaymentValue.setHolderName(mPaymentsManagerMAVRAV.getBillHolderName());
            	checkBillPaymentValue.setPurposeDescription(mPaymentsManagerMAVRAV.getDescription());
            	checkBillPaymentValue.setCurrency("$");
            	...
            	checkBillPaymentValue.setDueDate(TimeUtil.getDateString(mPaymentsManagerMAVRAV.getDueTo(), TimeUtil.dateFormat2));
            	json=CheckBillPaymentJson.CheckBillPaymentReportProtocal(Contants.publicModel, checkBillPaymentValue);
            	json=post(json);
            	mCheckBillPaymentResponseModel =CheckBillPaymentJson.ParseCheckBillPaymentResponse(json);
            	return mCheckBillPaymentResponseModel;
    ...
    private void onInsertRecipient(ResponsePublicModel responsePublicModel) {
        if (responsePublicModel.isSuccess()) {
            ...
            DialogManager.createMessageDialog(
                    responsePublicModel.eventManagement.getErrorDescription(), activity).show();
        }
    }
    ...
    private void onConfirmResult(Object result) {
        switch (getPaymentType()) {
            ...
            case MAV_RAV:
            	...
            	confirmResult=mInsertBillPaymentModel.responsePublicModel.isSuccess();
            	 if (confirmResult) {
                     // DialogManager.createMessageDialog("Transaction success!",
                     // activity).show();
                     reset();
    ...
    private Object confirm() {
        switch (getPaymentType()) {
            ...
            case BLANK_BILL:
            case MAV_RAV:
            	String json= InsertBillPaymentJson.InsertBillPaymentReportProtocal(Contants.publicModel, mCheckBillPaymentResponseModel.getCheckBillPaymentValueModel(), generateOtp.getOtpKeySession(), generatePin(), Contants.OTP_CHANNEL_MAIL);
            	return InsertBillPaymentJson.ParseCheckBillPaymentResponse(json);
        }
        return null;
    }

    private ResponsePublicModel confirmBankTransfer(CheckTransferResponseModel checkTransfer) {
        AccountsModel accountsModel = generateAccountModel();
        double amountInt = generateAmount();
        String transferType = genereateTransferType();
        String description = generateDescription();
    ...
    private SimTopUpResponseModel confirmTranslationSimTopUp(CheckSimTopUpResponseModel checkSimTopUp) {
        AccountsModel accountsModel = generateAccountModel();
        PhoneRecipient pr = (PhoneRecipient)getPayee();
    ...
    private ResponsePublicModel confirmCardRecharge(CheckRechargeCardResponseModel checkRechargeCard) {
        ...
        String postData = InsertRechargeCardJson.InsertRechargeCardReportProtocal(
                Contants.publicModel, accountsModel.getAccountCode(), amountdouble,
                tmp_InfoCardsModel.getCardHash(), cr.getCardNumber(), description, cr.getName(),
                tmp_InfoCardsModel.getTitle(), generatePin(), generateOtp.getOtpKeySession(), Contants.OTP_CHANNEL_MAIL,
                "1000",checkRechargeCard);
        HttpConnector httpConnector = new HttpConnector();
        String httpResult = httpConnector
                .requestByHttpPost(Contants.mobile_url, postData, activity);
    ...
    protected void startAddItem(int requestCode, int flag, String accountCode, Serializable payee) {
        ...
        switch (v.getId()) {
            ...
            case R.id.proceed_btn:
                if (getPaymentStep() == R.id.new_payments_step1_rb) {
                    ...
                    if (curType == 0) {
                        Toast.makeText(activity, R.string.pls_select_payments_type, Toast.LENGTH_LONG).show();
                        break;
                    }
                    double generateAmount=generateAmount();
                    switch (curType) {
                        case TRANSFER_ENTRY:
                        	mGaTracker1.sendView("event.newpayment.type.transfer.entry");
                        case BANK_TRANSFER:
                        	 mGaTracker1.sendView("event.newpayment.type.bank.transfer");

                             if (generateAmount == 0) {
                                 Toast.makeText(activity, R.string.missing_amount, Toast.LENGTH_LONG).show();
                                 break;
                             }
                             if(getPayee()==null){
                                 	Toast.makeText(activity, "select correct payee!", Toast.LENGTH_SHORT).show();
                            ...
                            if(generateDate()<=0){
                                Toast.makeText(activity, R.string.missing_date, Toast.LENGTH_LONG).show();
                                break; 
                            }
                            if(TextUtils.isEmpty(generateDescription())) {
                                Toast.makeText(activity, R.string.missing_description, Toast.LENGTH_LONG).show();
                                break;
                            }
                            setPaymentStep(R.id.new_payments_step2_rb);
                            break;
                        case CARD_TOP_UP:
                            ...
                            if (tmp_isVerifyCard != VERIFYCARD_SUCCESS) {
                                downloading_pd.show();
                                new PostThread(activity, null, null, mHandler,
                                        PaymentsManager.VERTIFY_CARD, PostThread.TYPE_VERTIFY_CARD)
                                        .start();
                            } else {
                                setPaymentStep(R.id.new_payments_step2_rb);
                            }
...
//                            }
                            ...
                            break;
                        case PRECOMPILED_BILL:
                        	if(TextUtils.isEmpty(mPaymentsManagerBill.getAccountNumber())){
                        	...
                        	break;
                        case BLANK_BILL:
                        	if(TextUtils.isEmpty(mPaymentsManagerBlankBill.getAccountNumber())){
                            activity.displayErrorMessage(activity
                                    .getString(R.string.missing_account_number));
                        	...
                        	break;
                        case MAV_RAV:
                        	if(TextUtils.isEmpty(mPaymentsManagerMAVRAV.getBillNumber())){
                                activity.displayErrorMessage(activity
                    ...
                    if (pin_lin.getVisibility() != View.VISIBLE) {
                        downloading_pd.show();
                        new PostThread(activity, null, null, mHandler, PaymentsManager.VALIDATE,
                                PostThread.TYPE_VALIDATE).start();
                    } else {
                        if (TextUtils.isEmpty(pin_et.getText())) {
                            Toast.makeText(activity, R.string.missing_pin, Toast.LENGTH_SHORT).show();
                            return;
                        }
                        downloading_pd.show();
                        new PostThread(activity, null, null, mHandler, PaymentsManager.CONFIRM,
                                PostThread.TYPE_CONFIRM).start();
                    }
                }
                break;
...
//                }
                if (payment == BANK_TRANSFER && arg2 == arg0.getCount() - 2) {
                    ...
                    startAddItem(NewPayee.NEW_TRANSFER_PAYEE, NewPayee.NEW_TRANSFER_PAYEE, tmp_payee);
                } else if (payment == BANK_TRANSFER && arg2 == arg0.getCount() - 1) { 
                	isTransferByPhone=true;
                    startAddItem(NewPayee.NEW_TRANSFER_PAYEE_BY_PHONE, NewPayee.NEW_TRANSFER_PAYEE_BY_PHONE, tmp_payee_phone);
                }else if (payment == CARD_TOP_UP && arg2 == arg0.getCount() - 1) {
                    startAddItem(NewPayee.NEW_CARD_PAYEE, NewPayee.NEW_CARD_PAYEE, generateAccountModel()
                            .getAccountCode(), tmp_payee);
                }else{
                    if(payment==CARD_TOP_UP){
                        setFoucs(description_et);
                    }else if(payment==BANK_TRANSFER||payment==TRANSFER_ENTRY){
            ...
            case R.id.phone_payee_folders_cf:
                ...
                if (arg2 == arg0.getCount() - 1) {
                    startAddItem(NewPayee.NEW_SIM_PAYEE, NewPayee.NEW_SIM_PAYEE, tmp_payee);
                } else
    ...
    private void loadAccountData(int paymentsType) {
        switch (paymentsType) {
            case GET_BANK_TRANSFER:
                downloading_pd.show();
                new PostThread(activity, null, null, mHandler, PaymentsManager.GET_BANK_TRANSFER,
                        PostThread.TYPE_BANK_TRANSFER).start();

                break;
            case GET_TRANSFER_ENTRY:
                downloading_pd.show();
                new PostThread(activity, null, null, mHandler, PaymentsManager.GET_TRANSFER_ENTRY,
                        PostThread.TYPE_TRANSFER_ENTRY).start();

                break;
            case GET_CARD_TOP_UP:
                downloading_pd.show();
                new PostThread(activity, null, null, mHandler, PaymentsManager.GET_CARD_TOP_UP,
                        PostThread.TYPE_CARD_TOP_UP).start();

                break;
            case GET_PHONE_TOP_UP:
                downloading_pd.show();
                new PostThread(activity, null, null, mHandler, PaymentsManager.GET_PHONE_TOP_UP,
                        PostThread.TYPE_PHONE_TOP_UP).start();

                break;
            case GET_ACCOUNT:
                downloading_pd.show();
                new PostThread(activity, null, null, mHandler, PaymentsManager.GET_ACCOUNT,
                        PostThread.TYPE_GET_ACCOUNT).start();
                break;
        }
    }
    ...
    private void loadAmountAvilable() {
        downloading_pd.show();
        new PostThread(activity, null, null, mHandler, PaymentsManager.GET_AMOUNT_AVAILABLE,
    ...
    public boolean onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if(mPaymentsManagerBill.onActivityResult(requestCode, resultCode, data)){
        	return true;
        }
        if(mPaymentsManagerBlankBill.onActivityResult(requestCode, resultCode, data)){
        	return true;
        }
        if(mPaymentsManagerMAVRAV.onActivityResult(requestCode, resultCode, data)){
        	return true;
        }
        String name = data.getStringExtra(NewPayee.NAME), number = data
                .getStringExtra(NewPayee.NUMBER), operator = data.getStringExtra(NewPayee.OPERATOR);
        ...
        switch (requestCode) {
            ...
            case NewPayee.NEW_TRANSFER_PAYEE_BY_PHONE:
                AccountsModel mAccountsModel = (AccountsModel) data.getSerializableExtra(NewPayee.OBJ_DATA);
//                bankRecipient.setIbanCode(data.getStringExtra(NewPayee.NUMBER));
                ...
                if (!bank_payee_cf_ad.getDatas().contains(mAccountsModel)) {
                    ...
                    existIndex = bank_payee_cf_ad.getDatas().indexOf(mAccountsModel);
                }
                payeeGallery = payee_folders_cf;
                // insertRecipient(InsertRecipientJson.BANK, bankRecipient);
                break;
...
//                break;
            case NewPayee.NEW_CARD_PAYEE:
                ...
                tmp_isVerifyCard = data.getIntExtra(NewPayee.VERIFYCARD_STATE, VERIFYCARD_INITAL);
            ...
            case NewPayee.NEW_SIM_PAYEE:
                ...
                if (!phone_payee_cf_ad.getDatas().contains(phoneRecipient)) {
                    ...
                    existIndex = phone_payee_cf_ad.getDatas().indexOf(phoneRecipient);
                }
                payeeGallery = phone_payee_folders_cf;
                // insertRecipient(InsertRecipientJson.PHONE,
                // phoneRecipient);
                if(!TextUtils.isEmpty(phoneRecipient.getName())&&!TextUtils.isEmpty(phoneRecipient.getPhoneNumber())){
        ...
        if (existIndex >= 0) {
            Toast.makeText(activity, R.string.payee_exist, Toast.LENGTH_SHORT).show();
            ...
            new PostThread(activity, type + "", tmp_payee, mHandler,
                    PaymentsManager.INSERTRECIPIENT, PostThread.TYPE_INSERTRECIPIENT).start();
        } else if(payeeGallery!=null){
            CoverFlowViewAdapter mCoverFlowViewAdapter=(CoverFlowViewAdapter)payeeGallery.getAdapter();
            if(TextUtils.isEmpty(name)&&TextUtils.isEmpty(number)){
                mCoverFlowViewAdapter.setNewItem(null);
            }else{
            	if(requestCode==NewPayee.NEW_TRANSFER_PAYEE_BY_PHONE){
            		((NPBankRecipientAdapter)mCoverFlowViewAdapter).setPhonePayeeItem(tmp_payee_phone);
            	}else{
     ...
     * @return
     */
    private String formateAmount(String str){
        str = str.replace(".", "");
        str = str.replace(',', '.');
        try {
...
//            format.setCurrency(Currency.getInstance("EUR"));
    ...
    private synchronized void endAountTextInput() {
        if (amount_et != null && amount_et.getWindowToken() != null) {
        	if (KeyBoardUtils.hideSoftInputFromWindow(activity,amount_et.getWindowToken())) {
                checkAmountView();
            }
        }
    }
