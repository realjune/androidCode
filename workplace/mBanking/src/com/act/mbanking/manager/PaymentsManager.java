...
import com.act.mbanking.manager.view.adapter.CoverFlowViewAdapter;
...
public class PaymentsManager implements OnClickListener, OnItemSelectedListener,
    ...
    public PaymentsManager(MainActivity activity2) {
...
//        }
    ...
    private void inviliAccountView(int paymentsType) {
        ...
        if (showForm == FORM_ACCOUNT&&App.app.transferEntryAccounts!=null) {
            List<AccountsModel> ens=App.app.transferEntryAccounts.get(0).getAccounts();
            int count=-1;
            String src,des=null;
            if(ens!=null){
                count=ens.size();
...
//                        loadAccountData(GET_ACCOUNT);
                ...
                case BANK_TRANSFER:
                    if (App.app.bankTransferAccounts != null) {
                        account_folders_cf_ad.setDatas(App.app.bankTransferAccounts.get(0).getAccounts());
                        account_folders_cf_ad.notifyDataSetChanged();
                    } else {
//                        loadAccountData(GET_BANK_TRANSFER);
                    ...
                    if (App.app.getRecipientListModel != null) {
                        bank_payee_cf_ad.setDatas(App.app.getRecipientListModel.getBankRecipientList());
                ...
                case TRANSFER_ENTRY:
                    if (null != App.app.transferEntryAccounts) {
                        account_folders_cf_ad.setDatas(App.app.transferEntryAccounts.get(0).getAccounts());
                        account_folders_cf_ad.notifyDataSetChanged();

                        entry_payee_cf_ad.setDatas(App.app.transferEntryAccounts.get(0).getAccounts());
...
//                        loadAccountData(GET_TRANSFER_ENTRY);
                ...
                case CARD_TOP_UP:
                    if (null != App.app.chargeAccounts) {
                        account_folders_cf_ad.setDatas(App.app.chargeAccounts.get(0).getAccounts());
                        account_folders_cf_ad.notifyDataSetChanged();
                    } else {
//                        loadAccountData(GET_CARD_TOP_UP);
                    ...
                    if (App.app.getRecipientListModel != null) {
                        card_payee_cf_ad.setDatas(App.app.getRecipientListModel.getCardRecipientList());
                ...
                case PHONE_TOP_UP:
                    if (null != App.app.simTopUpAccounts) {
                        account_folders_cf_ad.setDatas(App.app.simTopUpAccounts.get(0).getAccounts());
                        account_folders_cf_ad.notifyDataSetChanged();
                    } else {
//                        loadAccountData(GET_PHONE_TOP_UP);
                    ...
                    if (App.app.getRecipientListModel != null) {
                        phone_payee_cf_ad.setDatas(App.app.getRecipientListModel.getPhoneRecipientList());
                        phone_payee_folders_cf.setAdapter(phone_payee_cf_ad);
                    }
                ...
                case GET_ACCOUNT:
                    ...
                    if (allAccounts != null && allAccounts.get(0) != null) {
                        ...
                        if (App.app.transferEntryAccounts == null) {
                            ...
                            App.app.transferEntryAccounts.add(mAccountsForServiceModel);
                            ...
                            destLs = App.app.transferEntryAccounts.get(0).getAccounts();
                            if (destLs == null) {
                                destLs = new ArrayList<AccountsModel>();
                                App.app.transferEntryAccounts.get(0).setAccounts(destLs);
                            } else {
                                destLs.clear();
                            }
                        }
...
//	                    }
                    ...
                    if(showForm==FORM_DEFAULT&&(App.app.bankTransferAccounts==null||App.app.bankTransferAccounts.size()<=0||App.app.bankTransferAccounts.get(0).getAccounts().size()==0)){
                    	payment_type_sp.setEnabled(false);
                    	Toast.makeText(activity, "none payer account!", Toast.LENGTH_LONG).show();
                    }else
                    if(App.app.bankTransferAccounts!=null&&App.app.bankTransferAccounts.size()>0&&App.app.bankTransferAccounts.get(0).getAccounts().size()<=1){
    ...
    public void initData() {
       ...
       if(showForm==FORM_RECENT_PAYMENT){
        	...
        	if (App.app.bankTransferAccounts == null || App.app.bankTransferAccounts.size() <= 0||App.app.bankTransferAccounts.get(0).getAccounts()==null||App.app.bankTransferAccounts.get(0).getAccounts().size()<=0){
            		loadAccountData(GET_ACCOUNT);
            }else{
            	//If the customer has only one account, the "transfer entry" option must be not visible: the customer hasn't to select the payment type "transfer entry". 
            	if(App.app.bankTransferAccounts!=null&&App.app.bankTransferAccounts.size()>0&&App.app.bankTransferAccounts.get(0).getAccounts().size()<=1){
...
//                displayPaymentRecap();
    ...
    private void showPaymentType(int visibility, int selectIndex) {
        ...
        if(App.app.bankTransferAccounts!=null&&App.app.bankTransferAccounts.size()<=1&&selectIndex==TRANSFER_ENTRY){
        	--selectIndex;
        }
    ...
    private int getPaymentType() {
        ...
        if (showForm != PaymentsManager.FORM_RECENT_PAYMENT) {
        	...
        	//If the customer has only one account, the "transfer entry" option must be not visible: the customer hasn't to select the payment type "transfer entry". 
        	if(App.app.bankTransferAccounts!=null&&App.app.bankTransferAccounts.size()<=1&&paymentType>=2){
        		++paymentType;
        	}
        }
        return paymentType;
        ...
        public void run() {
            ...
            switch (type) {
                ...
                case TYPE_GET_ACCOUNT:
                    ...
                    App.app.getRecipientListModel = NewPaymentDataUtils.getRecipientList(activity);
                    break;
                case TYPE_UPDATE_BALANCE:
...
//    private List<AccountsForServiceModel> bankTransferAccounts=App.app.bankTransferAccounts;

//    private List<AccountsForServiceModel> transferEntryAccounts=App.app.transferEntryAccounts;

//    private List<AccountsForServiceModel> simTopUpAccounts=App.app.simTopUpAccounts;

//    private List<AccountsForServiceModel> chargeAccounts=App.app.chargeAccounts;

//    private List<AccountsForServiceModel> transferEntryPayeeAccounts=App.app.transferEntryPayeeAccounts;

//    private GetRecipientListModel getRecipientListModel=App.app.getRecipientListModel;
    ...
    private void toSelectBeneficiary(BankRecipient beneficiary) {
        ...
        List<BankRecipient> beneficiarys = bank_payee_cf_ad.getDatas();
...
//                }
                if (payment == BANK_TRANSFER && arg2 == arg0.getCount() - 1) {
                    startAddItem(BANK_TRANSFER, NewPayee.NEW_TRANSFER_PAYEE, tmp_payee);
                } else if (payment == CARD_TOP_UP && arg2 == arg0.getCount() - 1) {
                    startAddItem(CARD_TOP_UP, NewPayee.NEW_CARD_PAYEE, generateAccountModel()
            ...
            case R.id.phone_payee_folders_cf:
                ...
                if (arg2 == arg0.getCount() - 1) {
                    startAddItem(PHONE_TOP_UP, NewPayee.NEW_SIM_PAYEE, tmp_payee);
                } else
