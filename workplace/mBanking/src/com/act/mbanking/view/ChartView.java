...
//    private long to;
    ...
    private RectLD generateChartModelwjx(int s,int e,List<ChartModelMapTool> currentChartModelMapTools,RectLD rectLD) {
    	Log.d(TAG,"generateChartModelwjx");

    	if(rectLD==null){
    		rectLD=new RectLD();
    	}
    	if(currentChartModelMapTools==null&&currentChartModelMapTools.size()<0){
    	...
    	for(int i=s;i<e;++i){
        	...
        	for(int j=0;j<yNum;j++){
				...
				if(chartModelMapTool!=null){
					...
					if(chartModelMapTool.getYearList()!=null&&i<chartModelMapTool.getYearList().size()){
					...
					if(mChartModel!=null){
						...
						rectLD.top = Math.min(mChartModel.getAfterAdded(), rectLD.top);
		    			rectLD.bottom = Math.max(mChartModel.getAfterAdded(), rectLD.bottom);
					}
				}
...
//        ChartModelManager.writeSdcardFile("/sdcard/generateChartModelwjx.txt",str.toString().getBytes());
    ...
    public void setTimeStyle(int s,int e,int style) {
    	...
    	this.s=s;this.e=e;currentStyle=style;
...
//            public void onProgress() {
                ...
                Log.d(TAG,"Time from1:"+TimeUtil.getDateString(dataArea.left, TimeUtil.dateFormat2)+" to:"+TimeUtil.getDateString(dataArea.right, TimeUtil.dateFormat2));
...
//                e=366;
                ...
                dataArea=generateChartModelwjx(s,e,currentChartModelMapTools,dataArea);
...
//        });
     ...
     * @param style
     */
    public void initXValue(long from,long to,int style) {
...
//        currentStyle = style;
        ...
        switch (style) {
            case YEAR:
                ...
                for (int i = 0; i < xValues.length; i++) {
                    ...
                    xValues[i] = new ValueWidthName(TimeUtil.getDateString(temp.getTimeInMillis(),
                            TimeUtil.dateFormat12), temp.getTimeInMillis());
            ...
            case MONTH:
                ...
                for (int i = 0; i < xValues.length; i++) {
                    ...
                    xValues[i] = new ValueWidthName(TimeUtil.getDateString(temp.getTimeInMillis(),
                            TimeUtil.dateFormat14), temp.getTimeInMillis());
                    temp.set(Calendar.WEEK_OF_MONTH, temp.get(Calendar.WEEK_OF_MONTH) + 1);
                }
    ...
    public static RectLD getTimeStyleArea(int style,RectLD r) {
    	...
    	switch (style) {
    	case YEAR:
    		fromCalendar.set(Calendar.MONTH, fromCalendar.get(Calendar.MONTH) - 12);
    		fromCalendar.set(Calendar.HOUR_OF_DAY, 0);
    		fromCalendar.set(Calendar.MINUTE, 0);
    		fromCalendar.set(Calendar.SECOND, 0);
    		break;
    	case HALF_YEAR:
    		fromCalendar.set(Calendar.MONTH, fromCalendar.get(Calendar.MONTH) - 6);
    		fromCalendar.set(Calendar.HOUR_OF_DAY, 0);
    		fromCalendar.set(Calendar.MINUTE, 0);
    		fromCalendar.set(Calendar.SECOND, 0);
    		break;
    	case MONTH:
    		fromCalendar.set(Calendar.MONTH, fromCalendar.get(Calendar.MONTH) - 1);
    	...
    	Log.d(TAG,"Time from1:"+TimeUtil.getDateString(r.left, TimeUtil.dateFormat2)+" to:"+TimeUtil.getDateString(r.right, TimeUtil.dateFormat2));
    	return r;
    }
    ...
    private void drawNodeText(List<NodeTextItem> list, Canvas canvas) {
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            float parentY = nodeTextItem.draw(canvas);
            if (i + 1 < list.size()) {
    ...
    public float getSaftyX(float beginX) {
        BitmapDrawable bitmapDrawable = (BitmapDrawable)getContext().getResources().getDrawable(
                R.drawable.node);

        float end = o_x + xAxisLength - bitmapDrawable.getBitmap().getWidth() / 2;
        float x = beginX;
    ...
    private List<NodeTextItem> drawNodes(float beginX, Canvas canvas) {
        ...
        String fromStr = TimeUtil.getDateString(dataArea.left, TimeUtil.dateFormat2);
        String toStr = TimeUtil.getDateString(dataArea.right, TimeUtil.dateFormat2);
...
//        String key = TimeUtil.getDateString(time, TimeUtil.dateFormat4);
        ...
        for (int i = 0; i < currentChartModelMapTools.size(); i++) {
            ...
            ChartModelMapTool chartModelMapTool = currentChartModelMapTools.get(i);
            ...
            if(index<chartModelMapTool.getYearList().size()){
            	...
            	chartModel=chartModelMapTool.getYearList().get(chartModelMapTool.getYearList().size()-1);
            }
    ...
    private void drawSelect(float beginX, Canvas canvas) {
        ...
        BitmapDrawable drawable = (BitmapDrawable)getContext().getResources().getDrawable(
                R.drawable.timescale_select);
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                ...
                if (mTouchState == DOWN_1) {
                    ...
                    if (showStatusBeforeDown == show_1 || showStatusBeforeDown == show_2) {
                        showStatus = show_0;
                        invalidate();
...
//        cellLengthY = Math .abs(yAxisLength*0.9 / (yValues[0].value - yValues[yValues.length - 1].value));
//        float oo_y=(float) (yAxisLength*(0d-yValues[yValues.length - 1].value)/(yValues[0].value - yValues[yValues.length - 1].value));
//        
//        // 先排序
//        if(currentChartModelMapTools == null && currentChartModelMapTools.size() ==0){
//            return;
...
//                if(chartModel==null||yValues==null|| yValues.length<=0||yValues[0]==null){
//                	continue;
//                }
//                float yy = (float)(o_y - (chartModel.getAfterAdded() - yValues[0].value) * cellLengthY);
//
//                if (i == 0) {
//                    path.lineTo(o_x, yy);
//
...
//    }
     ...
     * 创建Path
     */
    private void createChartPath(int s,int e,List<ChartModelMapTool> currentChartModelMapTools){
        ...
        cellLengthX = xAxisLength / (float)(dataArea.width()-TimeUtil.ONE_DAY);
    		...
    		for(int j=0;j<yNum;j++){
        		...
        		}
    			{
    				...
    				if(chartModelMapTool!=null){
    					...
    					if(mChartModel!=null){
    						xp=(float)(o_x + (mChartModel.getTimeMills() - dataArea.left) * cellLengthX);
//    						if(mChartModel.getValue()<0){
//    							yp-=mChartModel.getValue();
//    							v=yp;
//    						}else{
...
//    						v=y_end+(float)(cellLengthY*(mChartModel.getAfterAdded()-yValues[yValues.length - 1].value));
    						v=o_y-(float)(cellLengthY*(mChartModel.getAfterAdded()-yValues[0].value));
    					}
    				}
...
//    				paths2[j].lineTo(xp, yp2);
    ...
    public void printValues(ValueWidthName[] valueWidthNames) {
     ...
     * @return
     */
//    public boolean isInArea(long time) {
...
//    }
    ...
    private RectLD calcDataArea(RectLD rectLD){
    	...
    	rectLD.bottom=Double.MIN_VALUE;
    	for (int j = 0; j < currentChartModelMapTools.size(); j++) {
    		List<ChartModel> models = currentChartModelMapTools.get(j).getYearList();
    		if (models == null) {
    			continue;
    		}
    		for (int i = 0; i < models.size(); i++) {
    			rectLD.top = Math.min(models.get(i).getAfterAdded(), rectLD.top);
    			rectLD.bottom = Math.max(models.get(i).getAfterAdded(), rectLD.bottom);
    		}
    	}
    	return rectLD;
    }
    ...
    @Deprecated
    public void generateYValues(double min,double max) {
        if (min >= 0) {

            yValuesStyle = 1;
            yValues = new ValueWidthName[NO_SIGN_SIZE];
            double cell = getCell(max, NO_SIGN_SIZE);
            // 计算Y轴上的数
            for (int i = 0; i < NO_SIGN_SIZE; i++) {
                String value = Utils.formatMoney(i * cell, "", false, true, false, false, true);
                yValues[i] = new ValueWidthName(value, (i * cell));
            }

        } else if (min < 0 && max > 0) {
            yValuesStyle = 0;
            yValues = new ValueWidthName[SIGN_SIZE];
            ...
            for (int i = -NO_SIGN_SIZE+1; i <= 0; i++) {
                String value = Utils.formatMoney(i * cell, "", false, true, false, false, true);
        ...
        min=Math.min(min, 0);
        max=Math.max(max, 0);
        double values[]=Number_scale.getVales(min, max, NO_SIGN_SIZE<<1, 0.01);
        yValues = new ValueWidthName[values.length];
        for (int i = 0; i < values.length; i++) {
            String value = Utils.formatMoney(values[i], "", false, true, false, false, true);
...
//        printValues(yValues);
    ...
    public void initYValues(double min,double max) {
...
//    }
    ...
    public static class NodeTextItem implements Comparable<NodeTextItem> {
        ...
        public float draw(Canvas canvas) {
            ...
            isRight = true;
            text = Utils.formatMoney(chartModel.getValue(), "$", true, true, false, false, true);
    ...
    public static class NodeTextItem2 extends NodeTextItem {
        ...
        public float draw(Canvas canvas) {
            ...
            String fromText = initialTag
                    + Utils.formatMoney(from.getValue(), "$", true, true, false, false, true);

            float fromLength = paint.measureText(fromText);

            String toText = finalTag
                    + Utils.formatMoney(to.getValue(), "$", true, true, false, false, true);

            if (finalTag.equals("FINAL TOTAL:")) {
                double present = 100 * (to.getValue() - from.getValue()) / to.getValue();
