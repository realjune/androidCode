...
import com.android.wjx.customview.activity.BankImageView;
...
public abstract class AbstractCoverFlowImageAdapter extends BaseAdapter {
    ...
    public final synchronized BankImageView getView(final int position, final View convertView,
        ...
        Bitmap bitmap = null;//getItem(position);
        if(position==this.position){
        	bitmap=createBitmap(position,true);
        }else{
        	bitmap=createBitmap(position,false);
        }
