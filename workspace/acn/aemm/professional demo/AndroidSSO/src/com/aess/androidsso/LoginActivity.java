...
public class LoginActivity extends Activity implements View.OnClickListener{
    ...
    private boolean mCancelLogin = true;
    ...
    public void onCreate(Bundle icicle) {
        ...
        mAppId = this.getIntent().getStringExtra("appid");
				...
				public void run() {
					...
					result = NetworkAuthenticate
			           //.authenticate(mUsername, mPassword, AuthenticatorActivity.this.getSharedPreferences(CommUtils.PREF_NAME, 0).getString(CommUtils.KEY_CONFIG_IMEI, ""), null/* Handler */, AuthenticatorActivity.this);
						.authenticate(mUsername, mPassword, mAppId, null/* Handler */, LoginActivity.this);
    ...
    public void onAuthenticationResult(int result) {
        ...
        if(mCanceled || mCancelLogin) {
    		Editor editor = getSharedPreferences("AEMM_CLIENT", 0).edit();
        ...
        if (result == 0) {
        	if(SSOService.mLoginResult == 0) {
        		SSOService.mLoginResult = 1;
        		try {
        			SSOService.mCallBack.loginResult(0);
	...
	private void cancel() {
		...
		if(SSOService.mLoginResult == 0) {
    		SSOService.mLoginResult = -1;
			try {
				SSOService.mCallBack.loginResult(-1);
			} catch (RemoteException e) {
			} catch (Exception e) {
			}
		}
