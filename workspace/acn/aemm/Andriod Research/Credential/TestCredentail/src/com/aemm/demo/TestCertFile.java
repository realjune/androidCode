...
public class TestCertFile extends PreferenceActivity implements FileFilter {
    ...
    protected List<File> getAllCertFiles() {
        ...
        Log.d(TAG, "install cert from " + "getAllCertFiles" + root.getAbsolutePath());
        File download = new File(root, DOWNLOAD_DIR);
        if (download != null) {
            File[] files = download.listFiles(this);
            Log.d(TAG, "install cert from " + "getAllCertFiles" + download.getAbsolutePath());
            if (files != null) Collections.addAll(allFiles, files);
        }

        File[] files = root.listFiles(this);
        if (files != null) Collections.addAll(allFiles, files);
       // Log.d(TAG, "install cert from " + "getAllCertFiles" + files.length);
    ...
    private void install(String fileName, byte[] value) {
    	Log.w(TAG, "install cert file is too large: ");
