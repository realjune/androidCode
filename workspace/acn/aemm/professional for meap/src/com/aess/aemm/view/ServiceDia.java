...
public class ServiceDia extends Activity{
	...
	public final static byte HALLLOCK_SCREENON=3;
	...
	public final int SetPasswordFlag = 1;
	 ...
	 public static void showAddApp(Context context,int strId){
		 ...
		 diaIntent.putExtra(COMMAND,ADDAPP);
          ...
          _Bundle.putInt(MESSAGE,strId);
          diaIntent.putExtras(_Bundle);                                    
		 context.startActivity(diaIntent);
	 }  
	 public static void showHallLockScreenOn(Context context,int strId){
		 Log.d(LOGCAT,"showHallLockScreenOn dialog");
		 Intent diaIntent;
		 diaIntent=(new Intent(context,ServiceDia.class));
		 diaIntent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
		 diaIntent.putExtra(COMMAND,HALLLOCK_SCREENON);
		  Bundle _Bundle = new Bundle();  
          _Bundle.putInt(MESSAGE,strId);
          diaIntent.putExtras(_Bundle);                                    
		 context.startActivity(diaIntent);
		...
		super.onCreate(savedInstanceState);
		 ...
		 this.getWindow().setFlags(WindowManager.LayoutParams.FLAG_SHOW_WHEN_LOCKED, WindowManager.LayoutParams.FLAG_SHOW_WHEN_LOCKED); 
			...
			if(com==WIFIOK){
		        ...
		        cd.show();
		        cd.setMessage(i.getIntExtra(MESSAGE,-1));
		        cd.addPositiveButton(R.string.open, new View.OnClickListener() {

					   @Override
					   public void onClick(View v) {
						   MainView.startHall(ServiceDia.this);
						   if(kl!=null) {
							   kl.reenableKeyguard();
						   }
//						   ViewUtils.update(ServiceDia.this, Messagedb.APP_UPDATE, null);
						   cd.dismiss();
						   ServiceDia.this.finish();
					   }
					  });
...
//	        	startActivityForResult(intent, SetPasswordFlag);
		        ...
		        cd.addPositiveButton(R.string.sure, new View.OnClickListener() {
					   ...
					   @Override
					   public void onClick(View v) {
						   PwPolicySetup da = PwPolicySetup.getInstance(ServiceDia.this);
							da.forceLock();
						   cd.dismiss();
						   ServiceDia.this.finish();
					   }
...
//	 @Override
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		if (requestCode == SetPasswordFlag) {
			...
			ComponentName mDeviceComponentName = new ComponentName(this, AemmDeviceAdminReceiver.class);
...
//	        kl.reenableKeyguard();
	 ...
	 public static  void test(final Context c){
		 new Handler().postDelayed(new Runnable(){
			...
			@Override
			public void run() {
				showAddApp(c, R.string.new_app_load);
			}
