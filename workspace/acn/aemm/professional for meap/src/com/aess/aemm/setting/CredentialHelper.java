...
class CredentialHelper {
    ...
    CredentialHelper(Intent intent, String type) {
        ...
        if (null != type) {
        	if (Credentials.CERTIFICATE.equals(type)) {
        		...
        		parseCert(getData(Credentials.PKCS12));
        	}
        }
    ...
    synchronized void onSaveStates(Bundle outStates) {
        try {
            ...
            outStates.putByteArray(CERTS_KEY, Utils.toBytes(certs));
        } catch (Exception e) {
            // should not occur
        }
    }
	...
	void onRestoreStates(Bundle savedStates) {
        mBundle = (HashMap<String, byte[]>)savedStates.getSerializable(DATA_KEY);
        ...
        ArrayList<byte[]> certs =
                Utils.fromBytes(savedStates.getByteArray(CERTS_KEY));
...
*/
    ...
    Intent createSystemInstallIntent() {
        ...
        if (!mCaCerts.isEmpty()) {
            Object[] caCerts = mCaCerts.toArray(new X509Certificate[mCaCerts.size()]);
    ...
    //add out new intent
    Intent createTestCredentailInstallIntent() {
        Intent intent = new Intent(Credentials.SYSTEM_INSTALL_ACTION);
        // To prevent the private key from being sniffed, we explicitly spell
        // out the intent receiver class.
        intent.setClassName("com.aemm.demo",
                "com.aemm.demo.TestCredentailInstaller");
        if (mUserKey != null) {
            intent.putExtra(Credentials.USER_PRIVATE_KEY + mName,
                    convertToPem(mUserKey));
        }
        ...
        if (!mCaCerts.isEmpty()) {
            ...
            intent.putExtra(Credentials.CA_CERTIFICATE + mName,
                    convertToPem(caCerts));
            Log.i("TestAemm",Credentials.CA_CERTIFICATE + ":"+ mName +":" + convertToPem(caCerts) );
        }
