...
import com.act.mbanking.bean.DataBaseModel;
...
public class BankSqliteHelper extends SQLiteOpenHelper {
    ...
    public static BankSqliteHelper getInstance(Context context) {

        if (bankSqliteHelper == null) {
            bankSqliteHelper = new BankSqliteHelper(context);
        }
...
//        log("sql:" + sql);
        SQLiteDatabase sqlitDatabase=getReadableDatabase();
        Cursor cursor =sqlitDatabase .rawQuery(sql, null);
        List<DataBaseModel> list = readCursor(table, cursor);
        log("database:query in database  table name=" + table + "size=" + list.size());
        if (cursor != null) {
            cursor.close();
