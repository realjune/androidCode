...
public class FlipCards {
  ...
  public FlipCards(FlipViewController controller, int orientation) {
    this.controller = controller;
    this.orientation = orientation;
    frontCards = new ViewDualCards(getOrientationVertical());
    ...
    switch (state) {
      ...
      case STATE_AUTO_ROTATE: {
        ...
        if (oldAngle < 0) { // bouncing back after flip backward and over the first page
          ...
          if (frontCards.getIndex() == maxIndex - 1 && oldAngle > frontCards.getIndex() * 180) { // bouncing
                                                                                                 // back
                                                                                                 // after
                                                                                                 // flip
                                                                                                 // forward
  ...
  public synchronized boolean handleTouchEvent(MotionEvent event, boolean isOnTouchEvent) {
    switch (event.getAction()) {
      case MotionEvent.ACTION_DOWN:
        ...
        lastPosition = getOrientationVertical() ? event.getY() : event.getX();
        return isOnTouchEvent;
      case MotionEvent.ACTION_MOVE:
        float delta =
            getOrientationVertical() ? (lastPosition - event.getY())
                : (lastPosition - event.getX());

        Log.e("FlipCards", "FlipCards:delta is " + delta);
        ...
        if (state == STATE_TOUCH) {
          ...
          Log.e("FlipCards", "FlipCards:forward is " + forward);
          ...
          if (Math.abs(angleDelta) > MAX_TOUCH_MOVE_ANGLE) // prevent large delta when moving too
                                                           // fast
          {
          ...
          if (frontCards.getIndex() == maxIndex - 1) { // the last page
            if (accumulatedAngle > frontCards.getIndex() * 180) {
              accumulatedAngle =
                  Math.min(
                      accumulatedAngle,
                      controller.isOverFlipEnabled() ? (frontCards.getIndex() * 180 + MAX_TIP_ANGLE)
          ...
          Log.e("FlipCards", "FlipCards:anglePageIndex is " + anglePageIndex);
                ...
                throw new RuntimeException(
                    AphidLog
                        .format(
                            ...
                            anglePageIndex, accumulatedAngle, frontCards.getIndex(),
                            backCards.getIndex()));
              }
            }
          }
  ...
  public boolean handleAutoEvent(/*FlipViewController controller*/) {
    synchronized (controller) {

      // case MotionEvent.ACTION_DOWN:
      // remember page we started on...
      lastPageIndex = getPageIndexFromAngle(accumulatedAngle);
      Log.e("FlipCards", "FlipCards:lastPageIndex is " + lastPageIndex);
      /*
       * if(orientation==1 && lastPageIndex==0){ lastPageIndex=2; }else
       */if (lastPageIndex == 1) {
        orientation = 1;
      }
      ...
      // delta = 10.0f; //delta>0: from top to bottom
      ...
      if (state == STATE_TOUCH) {
        ...
        Log.e("FlipCards", "accumulatedAngle is " + accumulatedAngle);
      ...
      Log.e(
          "FlipCards",
          "FlipCards:handleAuto:state is "
              + state
              + ","
              + "accumulatedAngle >= frontCards.getIndex() * 180"
              + "&& frontCards.getIndex() == maxIndex - 1 is "
              + (accumulatedAngle >= frontCards.getIndex() * 180 && frontCards.getIndex() == maxIndex - 1));
