...
public class FlipViewController extends AdapterView<Adapter> {
  ...
  public static final int VERTICAL_BOTTOM = 1; //open
  ...
  public FlipViewController(Context context, AttributeSet attrs, int defStyle) {
    ...
    try {
      int value = a.getInteger(R.styleable.FlipViewController_orientation, VERTICAL_TOP);
      if(value==VERTICAL_BOTTOM){
        orientation = VERTICAL_BOTTOM;
      }else if (value == HORIZONTAL_LEFT) {
  ...
  public boolean onAutoEvent(){
    if (flipByTouchEnabled) {
      return cards.handleAutoEvent();
    } else {
      return false;
    }
  }
  ...
  public void setAdapter(Adapter adapter) {
    if(this.flipOrientation==VERTICAL_BOTTOM){
      setAdapter(adapter, 0);
    }else{
      setAdapter(adapter, adapter.getCount()-1);
    }
  }
  ...
  private void setupSurfaceView(Context context) {
    ...
    cards = new FlipCards(this, flipOrientation);
  ...
  private void onDataChanged() {
    adapterDataCount = adapter.getCount();
    int activeIndex;
    Toast.makeText(getContext(), "adapterIndex:"+adapterIndex, 0).show();
    if (adapterIndex < 0) {
      activeIndex = 0;
    } else {
